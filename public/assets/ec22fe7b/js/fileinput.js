            ...
            fnSuccess = function (data, textStatus, jqXHR) {
                ...
                setTimeout(function () {
                    if (isEmpty(data) || isEmpty(data.error)) {
                        ...
                        //main!
                        if(!self.showNewPhoto){

                            $('.btn-file').hide();
                            $('#photos_title').hide();

                            var newImagePanel = $('<div>');
                            newImagePanel.attr('class', 'panel panel-default general newphotopanel');
                            newImagePanel.attr('id', 'newphotopanel');
                            newImagePanel = newImagePanel.appendTo('.file-input');

                            var newImagePanelHeading = $('<div>');
                            newImagePanelHeading.attr('class', 'panel-heading newphotoheading');
                            newImagePanelHeading.text('New photos');
                            newImagePanelHeading.appendTo(newImagePanel);

                            var newImagePanelBody = $('<div>');
                            newImagePanelBody.attr('class', 'panel-body newphotobody');
                            newImagePanelBody.appendTo(newImagePanel);

                            var newImageCloseButton = $('<button>');
                            newImageCloseButton.attr('type','button');
                            newImageCloseButton.attr('class','close');
                            newImageCloseButton.attr('data-target','#newphotopanel');
                            newImageCloseButton.attr('data-dismiss','alert');
                            newImageCloseButton.attr('aria-hidden','true');
                            newImageCloseButton.text('x');
                            newImageCloseButton.appendTo('.newphotoheading');
                            //кнопка закрытия окна только что добавленных фото
                            $('#newphotopanel button.close').click(function () {
                                self.showNewPhoto = false;
                                var descriptions = $('.newphotobody textarea');
                                for (i = 0; i < descriptions.length; i++) {
                                    var descr_id = descriptions[i].id;
                                    var descr_value = descriptions[i].value;
                                    if(descr_value != ""){
                                        $.ajax({
                                            url: "http://localhost/public/index.php?r=albums/save-photo-description",
                                            type: 'POST',
                                            dataType: 'text',
                                            data: ("id="+descr_id+"&value="+descr_value)
                                        });
                                    }
                                }
                                //обновить набор фото, выведенных на страницу
                                setTimeout(function () {
                                    $('#ajax_photos').click();
                            ...
                            self.showNewPhoto = true;
                        }
                        var response_data = JSON.parse(data);
                        var img = $('<img>');
                        img.attr('src', response_data['image_url']);
                        img.appendTo('.newphotobody');

                        var imgDesc = $('<textarea>');
                        imgDesc.attr('id', response_data['id']);
                        imgDesc.attr('class', 'form-control');
                        imgDesc.appendTo('.newphotobody');
                    } else {
                        self._showUploadError(data.error, params);
                        self._setPreviewError($thumb, i);
                        if (allFiles) {
        ...
        _readFiles: function (files) {
            ...
            readFile = function (i) {
                ...
                if (i >= numFiles) {
                    ...
                    $status.html('');
                    //changed
                    self.$container.find('.fileinput-upload').click();
